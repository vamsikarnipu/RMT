sap.ui.define([
    ...
    return Controller.extend("glassboard.controller.Home", {
        onInit() {
            ...
            CustomUtility.prototype._populateCountryDropdown.call(this);
        },
        ...
        onItemSelect: function (oEvent) {
            ...
            this._clearPreviousTableEditState(sKey).then((bAllowNavigation) => {
                ...
                CustomUtility.prototype._resetAllTablesToShowLess.call(this);
        ...
        // Extract fragment loading logic
        _loadFragmentIfNeeded: function (sKey, sPageId) {
            ...
            if (sKey === "customers") {
                ...
                // Remove existing content before adding new fragment to prevent duplicate IDs
                ...
                Fragment.load({
                    ...
                    CustomUtility.prototype._resetSegmentedButtonForFragment.call(this, "Customers");
                ...
                Fragment.load({
                    ...
                    // Reset segmented button to "less" state for this fragment
                    CustomUtility.prototype._resetSegmentedButtonForFragment.call(this, "Opportunities");
                ...
                // Check if content already exists and remove it to prevent duplicate IDs
                ...
                Fragment.load({
                    ...
                    // Reset segmented button to "less" state for this fragment
                    CustomUtility.prototype._resetSegmentedButtonForFragment.call(this, "Projects");
                ...
                Fragment.load({
                    ...
                    // Reset segmented button to "less" state for this fragment
                    CustomUtility.prototype._resetSegmentedButtonForFragment.call(this, "SAPIdStatuses");
                }.bind(this));
                ...
                Fragment.load({
                    ...
                    // Reset segmented button to "less" state for this fragment
                    CustomUtility.prototype._resetSegmentedButtonForFragment.call(this, "Employees");
                ...
                Fragment.load({
                    ...
                    // Reset segmented button to "less" state for this fragment
                    CustomUtility.prototype._resetSegmentedButtonForFragment.call(this, "Allocations");
                }.bind(this));
            }
        },
        // Reset functions moved to CustomUtility - use via delegation
        _resetAllTablesToShowLess: CustomUtility.prototype._resetAllTablesToShowLess,
        _resetAllSegmentedButtons: CustomUtility.prototype._resetAllSegmentedButtons,
        _resetSegmentedButtonForFragment: CustomUtility.prototype._resetSegmentedButtonForFragment,

        // Handle navigation with unsaved changes - Show confirmation dialog
        _clearPreviousTableEditState: function (sNewPageKey) {
            ...
            return new Promise((resolve) => {
                // Use custom action strings to ensure button text displays correctly
                    ...
                    `You have unsaved changes in ${sCurrentTable}. What would you like to do?`,
                    {
                        ...
                        onClose: (sAction) => {
                            ...
                            if (sAction === SAVE_ACTION) {
                                ...
                                // Call the actual cancel logic from CustomUtility to properly cancel OData changes
        ...
        // Save changes for a specific table
        ...
        // Cancel changes for a specific table (for navigation dialog)
        ...
        // Discard edit state for a specific table
        _discardTableEditState: function (sTableId) {
            ...
            // Clear form models to prevent pre-loading when navigating back
        ...
        // Handler for allocation view change (Employees/Projects toggle)
            ...
            if (sSelectedKey === "employees") {
                ...
                Fragment.load({
                    ...
                    if (oTable) {
                        ...
                        CustomUtility.prototype._resetSegmentedButtonForFragment.call(this, "Res");
        ...
        // Demand button handler - loads Demands fragment filtered by selected project
        onDemandPress: function() {
            ...
            Fragment.load({
                ...
                if (oDemandsTable) {
                    ...
                    // Apply filter after table is fully initialized
                    // Use multiple timeouts to ensure table is ready
                    setTimeout(() => {
                        ...
                        if (oBinding && sProjectId) {
                            ...
                            oBinding.filter([oFilter]);
                            console.log("✅ Filtered demands for project:", sProjectId);
                            
                            // Force refresh to ensure filter is applied
                            oBinding.refresh().then(() => {
                                console.log("✅ Demands table refreshed with filter");
                            }).catch((e) => {
                                console.warn("Error refreshing demands table:", e);
                            });
                        } else {
                            console.warn("Demands binding not ready or project ID missing");
                        }
                    }, 1000);
                    
                    CustomUtility.prototype._resetSegmentedButtonForFragment.call(this, "Demands");
                }
            }.bind(this));
        },
        
        // Back to Projects handler - returns to Allocations view
        ...
        // Resources handler - shows resources for selected demand
        onResourcesPress: function() {
            console.log("Find Resources");
            sap.m.MessageToast.show("Find Resources functionality - to be implemented");
            // TODO: Implement resources view
        },
        
        // Allocate Resource handler - opens allocation dialog
        ...
        // Search handler for Res (Employees) view
        onResSearch: function (oEvent) {
            ...
            if (oBinding) {
                if (sQuery) {
                    // Create search filter - search in fullName field
                    const oFilter = new sap.ui.model.Filter("fullName", sap.ui.model.FilterOperator.Contains, sQuery);
                    oBinding.filter([oFilter]);
                } else {
                    // Clear filter if search is empty
                    oBinding.filter([]);
        ...
        // Search handler for Demands view
        onDemandSearch: function (oEvent) {
            ...
            if (oBinding) {
                ...
                if (this._sDemandProjectFilter) {
                    aFilters.push(new sap.ui.model.Filter("sapPId", sap.ui.model.FilterOperator.EQ, this._sDemandProjectFilter));
                }
        ...
        // Res fragment - Customer change handler (enables Opportunity)
        ...
        // Res fragment - Opportunity change handler (enables Project)
        ...
        // Res fragment - Project change handler (enables Demand)
        ...
        // Res fragment - Opportunity value help (filtered by Customer)
        ...
        // Res fragment - Project value help (filtered by Opportunity)
        ...
        // Res fragment - Demand value help (filtered by Project)
        ...
        // Hard refresh function moved to CustomUtility - use via delegation
        _hardRefreshTable: CustomUtility.prototype._hardRefreshTable,

        // Submit function that handles both Create and Update
        onSubmitCustomer: function () {
            const sCustId = this.byId("inputCustomerId").getValue(),
                ...
                sState = this.byId("inputCity").getSelectedKey(), // City is a Select control
            ...
            if (aSelectedContexts && aSelectedContexts.length > 0) {
                    ...
                    Object.keys(oUpdateEntry).forEach(sKey => {
                        ...
                        oModel.submitBatch("changesGroup")
                            .then(() => {
                                ...
                                // Hard refresh table to get fresh data from DB
                ...
                if (oBinding) {
                    ...
                    try {
                        ...
                        // Set all properties individually to ensure they're queued in batch group
                        Object.keys(oCreateEntry).forEach(sKey => {
                            oNewContext.setProperty(sKey, oCreateEntry[sKey]);
                        });
                        
                        // Check if batch group has pending changes before submitting
                        ...
                        oModel.submitBatch("changesGroup")
                            .then(() => {
                                ...
                                // Fetch fresh data from backend (not from UI form)
        ...
        // Initialize Customer ID field with next ID preview
            ...
            try {
                if (oTable) {
                    ...
                    if (!sNextId || sNextId === "C-0001") {
                        const oModel = this.getOwnerComponent().getModel();
                        if (oModel) {
                            // Use OData V4 bindList instead of oModel.read()
        ...
        // Search function for Customer table
        ...
        // Search function for Opportunity table
        ...
        // Search function for Project table
        ...
        // Cancel function to clear form and deselect table row
        onCancelForm: function () {
            ...
            // Disable Edit button when form is cleared (no row selected)
        ...
        // Submit function for Employee (handles both Create and Update)
        onSubmitEmployee: function () {
            const sOHRId = this.byId("inputOHRId_emp").getValue(),
                ...
                sRole = this.byId("inputRole_emp").getSelectedKey(), // Role is a Select control
        ...
        // Submit function for Opportunity (handles both Create and Update)
        ...
        // Cancel function for Opportunity form
        ...
        // Initialize Opportunity ID field with next ID preview
        ...
        // Submit function for Project (handles both Create and Update)
        ...
        // Cancel function for Project form
        ...
        // Initialize Project ID field with next ID preview
        ...
        // Edit button handlers - populate forms when Edit is clicked
        ...
        // Cancel function for Employee form
        ...
        // Populate Country dropdown function moved to CustomUtility - use via delegation
        _populateCountryDropdown: CustomUtility.prototype._populateCountryDropdown,
